...
square_meter <- 2516.67
var_exp <- 126.44
# removing NaN
listings_full <- listings[complete.cases(listings), ]
bool_index <- listings_full$city==selected_city
# fixed costs
fixed_cost<-(square_meter*listings_full[bool_index,]$bedrooms* 10)
listings_full[bool_index, 'fixed_cost'] <- fixed_cost
# var costs
listings_full[bool_index, 'var_cost'] <- ((var_exp/31) * (365-as.integer(listings_full[bool_index, ]$availability_365)))
